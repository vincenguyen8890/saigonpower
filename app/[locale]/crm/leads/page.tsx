...
import type { LeadStatus, ServiceType } from '@/data/mock-crm'
...
export default async function LeadsPage({ params, searchParams }: Props) {
  ...
  if (status && status !== 'all') {
    leads = leads.filter(l => l.status === status as LeadStatus)
  }
  if (service && service !== 'all') {
    leads = leads.filter(l => l.service_type === service as ServiceType)
  }
  if (q) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {isVi ? 'Quản Lý Khách Hàng' : 'Leads Management'}
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            {leads.length} {isVi ? 'khách hàng' : 'leads'} {status && status !== 'all' ? `· ${status}` : ''}
          </p>
        </div>
      </div>
        ...
        <form className="flex flex-wrap gap-3 items-center">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              name="q"
              defaultValue={q}
              placeholder={isVi ? 'Tìm kiếm tên, email, SĐT...' : 'Search name, email, phone...'}
        ...
        {leads.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <p className="text-lg font-medium">
              {isVi ? 'Không tìm thấy khách hàng' : 'No leads found'}
            </p>
            <Link href={`/${locale}/crm/leads`} className="text-brand-green text-sm mt-2 inline-block">
              {isVi ? 'Xóa bộ lọc' : 'Clear filters'}
            </Link>
          </div>
        ) : (
