      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {plans.length} plans shown · Best rate: {(bestRate * 100).toFixed(1)}¢/kWh
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-gray-100 bg-gray-50/50">
              <tr>
                {['Plan', 'Provider', 'Rate', 'Term', 'Type', 'Cancel Fee', 'Promo'].map((h, i) => (
                  <th
                    key={i}
                    className={`text-left text-xs font-medium text-gray-400 uppercase tracking-wide px-5 py-3 ${i >= 5 ? 'hidden lg:table-cell' : ''} ${i === 4 ? 'hidden md:table-cell' : ''}`}
                  >
                    {h}
                  </th>
                ))}
