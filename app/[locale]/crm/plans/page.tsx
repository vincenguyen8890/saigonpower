...
export default async function PlansPage({ params, searchParams }: Props) {
  ...
  let plans = mockPlans.filter(p => p.status === 'active')
  ...
  const providers = [...new Set(mockPlans.map(p => ({ id: p.provider_id, name: p.provider_name })))]
  ...
  const bestRate = Math.min(...plans.map(p => p.rate_kwh))

  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isVi ? 'Gói Điện' : 'Energy Plans'}</h1>
          <p className="text-gray-500 text-sm mt-1">
            {plans.length} {isVi ? 'gói đang hiển thị' : 'plans shown'} · {isVi ? 'Tốt nhất' : 'Best rate'}: {(bestRate * 100).toFixed(1)}¢/kWh
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-5">
        <form className="flex flex-wrap gap-3 items-center">
          ...
          <select name="provider" defaultValue={provider || 'all'}
            className="border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-green">
            <option value="all">{isVi ? 'Tất Cả Nhà Cung Cấp' : 'All Providers'}</option>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-gray-100 bg-gray-50/50">
              <tr>
                {[isVi?'Gói Điện':'Plan', isVi?'Nhà Cung Cấp':'Provider', isVi?'Giá':'Rate', isVi?'Thời Hạn':'Term', isVi?'Loại':'Type', isVi?'Phí Hủy':'Cancel Fee', isVi?'Ưu Đãi':'Promo'].map((h, i) => (
                  <th key={i} className={`text-left text-xs font-medium text-gray-400 uppercase tracking-wide px-5 py-3 ${i >= 5 ? 'hidden lg:table-cell' : ''} ${i === 4 ? 'hidden md:table-cell' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
                  ...
                  <tr key={plan.id} className={`hover:bg-gray-50/60 transition-colors ${isBest ? 'bg-green-50/30' : ''}`}>
                    ...
                    <td className="px-5 py-4">
                      <span className="text-sm text-gray-700">
                        {plan.term_months === 0 ? (isVi ? 'Trả Trước' : 'Prepaid') : `${plan.term_months} ${isVi ? 'tháng' : 'mo.'}`}
                      </span>
                    </td>
                    <td className="px-5 py-4 hidden md:table-cell">
                      <span className={`text-xs px-2 py-0.5 rounded font-medium ${
                        ...
                        {plan.service_type === 'commercial' ? (isVi ? 'TM' : 'Comm.') : (isVi ? 'DC' : 'Res.')}
                      </span>
                    </td>
                    <td className="px-5 py-4 hidden lg:table-cell">
                      <span className="text-sm text-gray-600">
                        {plan.cancellation_fee != null ? `$${plan.cancellation_fee}` : (isVi ? 'Không có' : 'None')}
                      </span>
                    </td>
                    <td className="px-5 py-4 hidden lg:table-cell">
                      {plan.promo
